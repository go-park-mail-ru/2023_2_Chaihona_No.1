...
docker_build: 
	docker build -t m0rdovorot/kopilka.api:latest -f API.Dockerfile .;
	docker build -t m0rdovorot/kopilka.auth:latest -f auth.Dockerfile .;
	docker build -t m0rdovorot/kopilka.posts:latest -f posts.Dockerfile .;
	docker build -t m0rdovorot/kopilka.pay:latest -f pay.Dockerfile .;
docker_run:
